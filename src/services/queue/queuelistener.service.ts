...
export class QueueListenerService {
     ...
     * initialize aws queueSMS listener
     * 
     * @memberof QueueService
     */
    public init() {
     ...
     */
    private listen(): void {
        // If you want to remove all the messages from the queueSMS on every new startup
        this.sqs.purgeQueue({
    ...
    /**
     * Create new queueSMS if no queueu exists
     * 
     * @private
     * @returns 
    ...
    private createNewQueue() {
        const params = {
            ...
            Attributes: {
                'FifoQueue': 'true',
                'RedrivePolicy':
                    '{\"deadLetterTargetArn\":\"arn:aws:queue:eu-west-1:336081765760:TLD-test-deadletter.fifo\",\"maxReceiveCount\":\"10\"}',
            }
        };
